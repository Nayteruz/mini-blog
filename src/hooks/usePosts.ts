...
import type { ISortType } from "./types";
...
export const usePosts = () => {
  ...
  const createPost = async (postData: Omit<ICreatePostData, "createdAt">) => {
    try {
      ...
      const docRef = await addDoc(collection(db, DB_KEYS.POSTS), newPost);
  ...
  const deletePost = async (postId: string) => {
    try {
      ...
      await deleteDoc(doc(db, DB_KEYS.POSTS, postId));
      await loadPosts(); // Перезагружаем список постов
    } catch (err) {
  ...
  const updatePost = async (postId: string, updateData: IUpdatePostData) => {
    try {
      ...
      await updateDoc(doc(db, DB_KEYS.POSTS, postId), postUpdate);
      await loadPosts(); // Перезагружаем список постов
    } catch (err) {
